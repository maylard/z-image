...
def generate(
    ...
    for i, t in enumerate(tqdm(timesteps, desc="Denoising", total=len(timesteps))):
        ...
        # Process transformer input sequentially in chunks to save memory
        model_out_list = []
        for j in range(len(latent_model_input_list)):
            out = transformer(
                [latent_model_input_list[j]],
                timestep_model_input[j : j + 1],
                [prompt_embeds_model_input[j]],
            )[0]
            model_out_list.extend(out)
    ...
    # Decode latents sequentially to save memory
    decoded_images = []
    for i in range(latents.shape[0]):
        img = vae.decode(latents[i : i + 1], return_dict=False)[0]
